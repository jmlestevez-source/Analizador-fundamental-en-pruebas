...
export const handler = async (event) => {
  ...
  try {
    const [stockChart, sp500Chart] = await Promise.all([
    ...
    const technical = calculateTechnicalScore(symbol, stockSeries, sp500Series);
...
function calculateTechnicalScore(symbol, stockSeries, sp500Series) {
  ...
  const weeklyStock = toWeeklyCloses(stockSeries);
  const weeklySP500 = toWeeklyCloses(sp500Series);
  const rscMansfield = calculateMansfieldRSC(weeklyStock, weeklySP500, 52);
...
function calculateMansfieldRSC(stockWeekly, sp500Weekly, period = 52) {
  const aligned = alignByDate(stockWeekly, sp500Weekly);
  const rs = aligned.map(x => x.stockClose / x.sp500Close).filter(Number.isFinite);
...
function alignByDate(stockSeries, sp500Series) {
  const sp500ByDay = new Map(sp500Series.map(x => [toDateKey(x.date), x.close]));
  return stockSeries
    .map(stock => {
      const sp500Close = sp500ByDay.get(toDateKey(stock.date));
...
function toWeeklyCloses(dailySeries) {
  const weeks = new Map();
  for (const item of dailySeries) {
    const key = `${item.date.getUTCFullYear()}-${getISOWeek(item.date)}`;
    weeks.set(key, item);
  }
