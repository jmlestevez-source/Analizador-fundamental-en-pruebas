...
function App() {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const remoteSuggestions = (data.quotes || [])
          ...
          .map(q => ({
            ticker: String(q.symbol).replace(/\./g, '-'),
  ...
  const loadScreener = async () => {
    ...
    try {
      const response = await fetch(`/data/screener-top-50.json?ts=${Date.now()}`);
      if (!response.ok) throw new Error(`No se pudo cargar el screener (${response.status})`);
      const data = await response.json();
              ...
              <div className="suggestions-dropdown">
                {suggestions.map(s => (
                  <div 
                    ...
                    onClick={() => fetchStockData(s.ticker)}
                  >
